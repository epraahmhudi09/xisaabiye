...
export const LoanPaymentModal = ({ isOpen, onClose, customer }) => {
  ...
  return (
    <Modal isOpen={isOpen} onClose={onClose} title={`Receive Dayn Repayment - ${customer.name}`}>
      <form onSubmit={handleSubmit} className="space-y-4">
        ...
        <div className="p-4 rounded-xl bg-rose-950/40 border border-rose-800/60 flex items-center justify-between">
          <div>
            <span className="text-xs text-rose-300 font-medium block">Current Outstanding Debt (Dayn):</span>
            <span className="text-2xl font-black text-rose-400">{formatCurrency(currentDebt)}</span>
          </div>
          <button
            type="button"
            onClick={handleFullPayToggle}
            className="px-3 py-1.5 rounded-lg bg-rose-800 hover:bg-rose-700 text-xs font-bold text-white transition-colors"
          >
            Pay Full Debt
          </button>
        </div>
        ...
        <div>
          <label className="block text-xs font-semibold uppercase text-slate-400 mb-1">
            Payment Notes / Receipt Ref (Optional)
          </label>
          <input
            type="text"
            value={notes}
            onChange={(e) => setNotes(e.target.value)}
            placeholder="e.g. Cash handed over in shop"
        ...
        <div className="flex items-center justify-end gap-3 pt-3 border-t border-slate-800">
          ...
          >
            ...
            <span>{submitting ? 'Recording...' : 'Confirm Repayment'}</span>
          </button>
        </div>
      </form>
    </Modal>
