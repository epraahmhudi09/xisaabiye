  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 glass-panel p-6 rounded-2xl border border-slate-200 dark:border-slate-800">
        <div>
          <h2 className="text-2xl font-extrabold text-slate-900 dark:text-white tracking-tight">Profit, Loss & Financial Reports</h2>
          <p className="text-xs text-slate-600 dark:text-slate-400 mt-1 font-medium">
            Analyze profit margins, filter by custom date ranges, and export structured Excel or PDF reports.
          </p>
        </div>
        <div className="flex items-center gap-3">
          <button
            onClick={handleExportExcel}
            className="px-4 py-2.5 rounded-xl bg-emerald-600 hover:bg-emerald-500 text-white font-bold text-xs sm:text-sm shadow-md flex items-center gap-2 transition-transform hover:scale-105"
          >
          ...
          <button
            ...
            className="px-4 py-2.5 rounded-xl bg-slate-100 dark:bg-slate-800 hover:bg-slate-200 dark:hover:bg-slate-700 text-slate-800 dark:text-slate-200 font-bold text-xs sm:text-sm border border-slate-200 dark:border-slate-700 shadow-md flex items-center gap-2 transition-transform hover:scale-105"
          >
      ...
      <div className="glass-panel p-5 rounded-2xl border border-slate-200 dark:border-slate-800 space-y-4">
        <div className="flex flex-col md:flex-row items-start md:items-center justify-between gap-4">
          <div className="flex items-center gap-2">
            ...
            <span className="text-xs font-bold uppercase text-slate-700 dark:text-slate-300">Date Range Filter:</span>
          </div>
        ...
        {dateRangeFilter === 'CUSTOM' && (
          <div className="flex items-center gap-4 pt-3 border-t border-slate-200 dark:border-slate-800/80 animate-in fade-in duration-200">
            <div className="flex items-center gap-2">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        <div className="p-5 rounded-2xl glass-panel border border-emerald-200 dark:border-emerald-500/30 bg-emerald-50/50 dark:bg-emerald-950/20">
          <span className="text-xs text-slate-600 dark:text-slate-400 font-bold uppercase tracking-wider block">Filtered Total Revenue</span>
          <h3 className="text-3xl font-black text-slate-900 dark:text-white mt-1">{formatCurrency(totalRevenue)}</h3>
          <p className="text-xs text-emerald-700 dark:text-emerald-400 font-semibold mt-1">Cost of Goods: ${totalCostOfGoods.toFixed(2)}</p>
        </div>
        ...
        <div className="p-5 rounded-2xl glass-panel border border-blue-200 dark:border-blue-500/30 bg-blue-50/50 dark:bg-blue-950/20">
          <span className="text-xs text-blue-700 dark:text-blue-300 font-bold uppercase tracking-wider block">Calculated Net Gross Profit</span>
          <h3 className="text-3xl font-black text-blue-600 dark:text-blue-400 mt-1">{formatCurrency(totalGrossProfit)}</h3>
          <p className="text-xs text-slate-600 dark:text-slate-400 font-medium mt-1">Margin: {totalRevenue > 0 ? ((totalGrossProfit / totalRevenue) * 100).toFixed(1) : 0}%</p>
        </div>
        ...
        <div className="p-5 rounded-2xl glass-panel border border-amber-200 dark:border-amber-500/30 bg-amber-50/50 dark:bg-amber-950/20">
          <span className="text-xs text-amber-700 dark:text-amber-300 font-bold uppercase tracking-wider block">Cash Settled</span>
          <h3 className="text-3xl font-black text-amber-600 dark:text-amber-400 mt-1">{formatCurrency(totalCashRecd)}</h3>
          <p className="text-xs text-slate-600 dark:text-slate-400 font-medium mt-1">Immediate cash liquidity</p>
        </div>
        ...
        <div className="p-5 rounded-2xl glass-panel border border-rose-200 dark:border-rose-500/40 bg-rose-50/50 dark:bg-rose-950/20">
          <span className="text-xs text-rose-700 dark:text-rose-300 font-bold uppercase tracking-wider block">Dayn Credit Issued</span>
          <h3 className="text-3xl font-black text-rose-600 dark:text-rose-400 mt-1">{formatCurrency(totalLoansInPeriod)}</h3>
          <p className="text-xs text-slate-600 dark:text-slate-400 font-medium mt-1">Total credit extended in range</p>
        </div>
      </div>
      ...
      <div className="glass-panel rounded-2xl border border-slate-200 dark:border-slate-800 overflow-hidden shadow-xl">
        <div className="p-5 border-b border-slate-200 dark:border-slate-800 flex items-center justify-between">
          <h3 className="font-extrabold text-base text-slate-900 dark:text-white">Filtered Sales Ledger & Itemized Profits</h3>
          <span className="text-xs font-semibold text-slate-600 dark:text-slate-400">{filteredSales.length} items listed</span>
        </div>
          ...
          <table className="w-full text-left border-collapse">
            <thead>
              <tr className="border-b border-slate-200 dark:border-slate-800 bg-slate-100 dark:bg-slate-950/80 text-[11px] font-extrabold uppercase text-slate-700 dark:text-slate-400 tracking-wider">
                ...
                <th className="py-4 px-6">Customer / Debtor</th>
              </tr>
            </thead>
              ...
              {filteredSales.length === 0 ? (
                <tr>
                  <td colSpan="9" className="py-12 text-center text-slate-500 font-medium">
                    No sales records found within selected date range.
                  </td>
                </tr>
              ) : (
